	...
	<section id="section">
		<form action="LogIn" method="post" onSubmit="return addParam(this);">
			<div class="id">
				...
				<span class="input__space"><input type="text"
					name="accessInfo" id="id" minlength="3" maxlength="10" title="ID"
					required></span>
			</div>
			<div class="pw">
				<h3 class="join__title">
				...
				<span class="input__space"><input type="password"
					name="accessInfo" id="password" minlength="4" maxlength="10"
					required></span>
			</div>
			...
			<span onClick="movePage(true)">[ Member Join ]</span> <span
				onClick="movePage(false)">[ Previous ]</span>
...
<script>
	...
	function addParam(form) {
		...
		form.action += "?action=${param.action}&gInfo=${paramValues.gInfo[0]}"
						+ "&gInfo=${paramValues.gInfo[1]}&gInfo=${paramValues.gInfo[2]}&opt=${paramValues.opt[0]}";
		return true;
	}
	...
	function movePage(selection) {
		...
		form.setAttribute("prev", page.name);
